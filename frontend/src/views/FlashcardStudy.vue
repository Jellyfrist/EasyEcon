  ...
  <div class="study-page">
    ...
    <template v-else-if="store.currentSet">
      ...
      <div v-else class="main-area">
        ...
        <p class="kb-hint">← Don't Know · → Know · Space to flip · use buttons to browse</p>
...
async function mark(status) {
  if (trackProgress.value) await store.markCard(currentCard.value.id, status)
...
.back-btn:hover { background: var(--gray-light); transform: translateX(-2px); }

.set-info { display: flex; align-items: center; gap: 6px; }
.set-label { font-size: 0.92rem; font-weight: 800; color: var(--primary-pink); }
.set-divider { color: var(--card-border); }
.set-name {
  font-size: 0.85rem;
...
.card-counter { font-size: 1.05rem; font-weight: 800; color: var(--text-main); }
.card-title-sm { font-size: 0.75rem; color: var(--text-muted); }
.topbar-right { flex: 1; }
...
.state-box .material-symbols-outlined { font-size: 48px; opacity: 0.35; }
...
.prog-chip.prog-learning { color: #f97316; }
.prog-chip.prog-known { color: #10b981; }
...
.prog-bar-learning { background: linear-gradient(90deg, #fb923c, #f97316); left: 0; }
.prog-bar-known { background: linear-gradient(90deg, #34d399, #10b981); }
...
.flashcard-wrap:hover { filter: drop-shadow(0 20px 48px rgba(46,56,86,0.18)); }
...
.flashcard-back {
  background: linear-gradient(145deg, #fff9f1, #fff4e6);
  border: 1.5px solid rgba(237,64,130,0.1);
  transform: rotateY(180deg);
}

.card-img-wrap { width: 100%; max-height: 160px; display: flex; justify-content: center; }
...
.shortcut-icon { font-size: 20px; color: var(--primary-pink); opacity: 0.8; }
.shortcut-label { font-weight: 700; color: var(--text-main); }
...
.hint-btn:hover { background: #fef3c7; box-shadow: 0 4px 12px rgba(251,191,36,0.25); }
.hint-btn-active { background: #fef3c7; }
.hint-btn .material-symbols-outlined { font-size: 16px; }
...
.hint-box .material-symbols-outlined { font-size: 18px; color: #f59e0b; flex-shrink: 0; }
.hint-slide-enter-active { transition: opacity 0.25s, transform 0.25s; }
.hint-slide-leave-active { transition: opacity 0.15s; }
.hint-slide-enter-from { opacity: 0; transform: translateY(6px); }
...
.track-toggle { display: flex; align-items: center; gap: 0.6rem; flex: 1; }
.track-label { font-size: 0.82rem; color: var(--text-muted); font-weight: 600; }
...
.toggle-btn.toggle-on { background: var(--primary-pink); }
...
.mark-btns { display: flex; gap: 1.25rem; flex: 1; justify-content: center; }
...
.mark-btn:hover:not(:disabled) { transform: scale(1.05); box-shadow: 0 8px 20px rgba(0,0,0,0.15); }
.mark-btn:active:not(:disabled) { transform: scale(0.97); }
.mark-btn .material-symbols-outlined { font-size: 20px; }
.mark-btn:disabled { opacity: 0.3; cursor: not-allowed; box-shadow: none; }

.mark-learning { background: linear-gradient(135deg, #fed7aa, #fdba74); color: #c2410c; }
.mark-learning:hover:not(:disabled), .mark-learning.active { background: linear-gradient(135deg, #fdba74, #fb923c); }
.mark-known { background: linear-gradient(135deg, #a7f3d0, #6ee7b7); color: #065f46; }
.mark-known:hover:not(:disabled), .mark-known.active { background: linear-gradient(135deg, #6ee7b7, #34d399); }
...
.next-btn:hover { background: var(--primary-hover); transform: translateY(-1px); box-shadow: 0 6px 18px rgba(237,64,130,0.45); }
.next-btn .material-symbols-outlined { font-size: 20px; }

.right-controls { display: flex; gap: 0.5rem; flex: 1; justify-content: flex-end; }
...
.icon-btn:hover { background: var(--white); color: var(--text-main); transform: scale(1.05); box-shadow: var(--shadow-md); }
...
.nav-btn:hover:not(:disabled) { background: var(--white); box-shadow: var(--shadow-md); }
.nav-btn:disabled { opacity: 0.3; cursor: not-allowed; }
.nav-btn .material-symbols-outlined { font-size: 26px; }

.dot-row { display: flex; gap: 6px; flex-wrap: wrap; justify-content: center; flex: 1; }
...
.dot-active { background: var(--primary-pink); border-color: var(--primary-pink); transform: scale(1.4); }
.dot-known { background: #34d399; border-color: #34d399; }
.dot-learning { background: #fb923c; border-color: #fb923c; }

.kb-hint { font-size: 0.75rem; color: var(--text-muted); text-align: center; opacity: 0.6; }
...
.done-icon { font-size: 4rem; line-height: 1; animation: pop 0.5s cubic-bezier(0.4,0,0.2,1); }
...
.done-screen h2 { font-size: 1.8rem; font-weight: 800; letter-spacing: -0.02em; }
.done-screen p { color: var(--text-muted); max-width: 380px; }

.done-actions { display: flex; gap: 1rem; flex-wrap: wrap; justify-content: center; margin-top: 0.5rem; }
...
@media (max-width: 600px) {
  .study-topbar { padding: 0.75rem 1rem; }
  .main-area { padding: 1.25rem 1rem 1.5rem; gap: 0.75rem; }
  .flashcard-wrap { height: 300px; }
  .card-word { font-size: 1.35rem; }
  .topbar-left .set-name { display: none; }
  .mark-btn { width: 52px; height: 52px; }
}
</style>