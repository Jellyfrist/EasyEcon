<template>
    <div class="dashboard">
            ...
            <div class="header">
                ...
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>
                    ...
                    <div class="header-text">
                        <p class="breadcrumb">
                            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg> Course #{{ courseId }}
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M9 18l6-6-6-6"/></svg> Exams
                        </p>
        ...
        <div v-else class="session-list">
            <div v-for="session in filteredSessions" :key="session.id" class="session-card" :class="{ 'is-open': session.is_open, 'is-closed': !session.is_open }">
                ...
                {{ session.is_open ? 'Open' : 'Closed' }}
                    ...
                    <button v-if="session.is_open && attemptsRemaining(session) > 0" class="btn-primary" @click="openExam(session)">
                {{ getMyAttempt(session.id) ? 'Retake' : 'Start Exam' }}
              </button>
    
                    <button v-else-if="getMyAttempt(session.id)" class="btn-ghost" @click="viewResult(session)">
                View Result
              </button>
