        ...
        <main class="main-container">
            ...
            <div class="page-header">
                <div class="title-area">
                    ...
                    <p class="page-subtitle">Manage all modules and lesson content for this course</p>
                </div>
            ...
            <div v-else-if="modules.length === 0" class="empty-state">
                ...
                <p class="empty-desc">No modules or lessons yet.<br>Click "Manage Modules" above to get started.</p>
            </div>
                ...
                <div v-for="(mod, index) in modules" :key="mod.id" class="module-card">
                    ...
                    chevron_right
                            ...
                            <h2 class="module-title">
                                <span class="module-badge">Module {{ index + 1 }}</span> {{ mod.title }}
                            </h2>
                        </div>
    
                        <button @click.stop="createNewLesson(mod.id)" class="btn-outline-primary">
                  <span class="material-symbols-outlined icon-small">add</span> Add Lesson
                </button>
                    </div>
                    ...
                    <div v-show="mod.isExpanded" class="module-content">
                        <div v-if="mod.learning_pages && mod.learning_pages.length > 0" class="lessons-list">
                            ...
                            <div v-for="(page, pIndex) in mod.learning_pages" :key="page.id" class="lesson-item" @click="editLesson(page.id, mod.id)">
                            ...
                            <span class="material-symbols-outlined icon-micro">done_all</span> Published
                                            </span>
                                            <span v-else class="status-badge draft">
                            <span class="material-symbols-outlined icon-micro">draft</span> Draft
                                            </span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div v-else class="empty-lessons">
                            <span class="material-symbols-outlined">info</span> No lessons in this module yet.
                        </div>
                    </div>
    ...
    try {
        ...
        for (let mod of fetchedModules) {
            try {
                ...
                console.warn(`Failed to fetch pages for module ${mod.id}`, err);
                mod.learning_pages = [];
                mod.isExpanded = false;
            }
        }
        ...
        error.value = "Network error — failed to load data"
    } finally {
        loading.value = false
    }
}
