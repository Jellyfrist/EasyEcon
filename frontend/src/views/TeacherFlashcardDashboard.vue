    ...
    <div v-else class="sets-grid">
      <div v-for="set in store.sets" :key="set.id" class="set-card" @click="goToStudy(set.id)">
        ...
        <div class="set-actions">
          <button class="btn btn-outline btn-sm" @click.stop="goToEditor(set.id)">
            <span class="material-symbols-outlined">edit</span>
            Edit
          </button>
          <button class="btn btn-danger-outline btn-sm" @click.stop="confirmDelete(set)">
...
.btn-danger-outline .material-symbols-outlined { font-size: 16px; }
...
.modal-title { font-size: 1.05rem; font-weight: 700; margin: 0 0 0.5rem; }
.modal-body { font-size: 0.875rem; color: var(--text-muted); margin: 0 0 1.5rem; }
.modal-actions { display: flex; justify-content: flex-end; gap: 0.75rem; }

@media (max-width: 600px) {
  .teacher-fc-page { padding: 1rem 1rem 3rem; }
