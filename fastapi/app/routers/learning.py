...
def create_page(
    ...
    '''
    - teacher creates a new lesson page
    - content blocks is a free form ordered list that can add any block types in any order
    - topic tag is stored but never exposed to students
    '''
    
    data = body.model_dump()
    data["content_blocks"] = [b.model_dump() for b in body.content_blocks]
    page = LearningPage(**data)
    db.add(page)
    db.commit()
    db.refresh(page)
    return page
